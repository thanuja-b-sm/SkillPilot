        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5">
              <div className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-xl space-y-4 relative">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-red-400" />
                    <div className="w-3 h-3 rounded-full bg-amber-400" />
                    <div className="w-3 h-3 rounded-full bg-emerald-400" />
                    <span className="text-xs font-bold text-slate-700 ml-2">SkillPilot Match Preview</span>
                  </div>
                  <span className="text-[11px] font-semibold bg-emerald-50 text-emerald-700 px-2.5 py-0.5 rounded-md border border-emerald-200">
                    System Calculated
                  </span>
                </div>

                {/* Sample Match Card inside Hero */}
                {careerMatches.slice(0, 2).map((match, idx) => (
                  <div key={match.career.id} className="p-4 rounded-xl bg-slate-50 border border-slate-200/70 hover:border-blue-300 transition-colors space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold uppercase text-slate-500 tracking-wider">Top Match #{idx + 1}</span>
                      <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">
                        {match.matchScore}% Match
                      </span>
                    </div>
                    <h3 className="text-base font-bold text-slate-900">{match.career.title}</h3>
                    <p className="text-xs text-slate-600 line-clamp-2">{match.career.description}</p>
                    <div className="flex items-center justify-between pt-1 text-[11px] text-slate-500">
                      <span>Salary: {match.career.averageSalary}</span>
                      <span className="text-emerald-600 font-medium">{match.career.growthRate}</span>
                    </div>
                  </div>
                ))}
                ...
                <button
                  ...
                  className="w-full py-2.5 text-xs font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors flex items-center justify-center gap-1"
                >
                  <span>View All {careers.length} Career Matches</span>
