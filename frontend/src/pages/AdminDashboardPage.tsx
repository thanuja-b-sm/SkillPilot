...
export const AdminDashboardPage: React.FC = () => {
  ...
  const [mailHealthData, setMailHealthData] = useState<Record<string, any> | null>(null);
  ...
  useEffect(() => {
    if (activeTab === 'overview' || activeTab === 'weights') {
      ...
      Promise.all([
        ...
        fetch('/api/admin/system/mail-health', { headers: { 'Authorization': `Bearer ${tok}` } }).then(r => r.ok ? r.json() : null).catch(() => null)
      ])
      .then(([stats, health, mailHealth]) => {
        if (stats) setAdminStats(stats);
        if (health) setHealthData(health);
        if (mailHealth) setMailHealthData(mailHealth);
      })
      ...
      <div className="lg:col-span-9 space-y-6">
        ...
        {activeTab === 'overview' && (
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-md space-y-6">
            ...
            {/* Mail Service Diagnostic Card */}
            {mailHealthData && (
              <div className="p-5 rounded-2xl border border-slate-200 bg-slate-50/80 space-y-3 text-left">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-slate-900">Email Relay Diagnostics ({mailHealthData.provider || 'Brevo SMTP'})</h4>
                      <p className="text-[10px] text-slate-500 font-medium">Relay: {mailHealthData.host}:{mailHealthData.port} • From: {mailHealthData.sender}</p>
                    </div>
                  </div>

                  <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 ${
                    mailHealthData.configured && !mailHealthData.lastFailure
                    ...
                    {mailHealthData.configured && !mailHealthData.lastFailure ? (
                      <>
                        ...
                        <span>SMTP HEALTHY</span>
                      </>
                    ) : (
                      <>
                        <AlertCircle className="w-3 h-3 text-rose-600" />
                        <span>SMTP FAILING</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-[11px]">
                  <div className="p-2.5 rounded-xl bg-white border border-slate-200">
                    <span className="text-slate-400 text-[10px] uppercase font-bold block">Last Successful Delivery</span>
                    <span className="text-slate-800 font-mono font-bold">
                      {mailHealthData.lastSuccessfulEmail ? new Date(mailHealthData.lastSuccessfulEmail).toLocaleString() : 'Ready / No recent dispatch'}
                    </span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white border border-slate-200">
                    <span className="text-slate-400 text-[10px] uppercase font-bold block">Last Failure Error</span>
                    <span className={`font-mono font-bold ${mailHealthData.lastFailure ? 'text-rose-600' : 'text-slate-400'}`}>
                      {mailHealthData.lastFailure || 'None (Zero Errors)'}
                    </span>
                  </div>
                </div>
              </div>
