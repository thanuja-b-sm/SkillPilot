...
public class EmailService {
    ...
    public boolean sendEmailVerificationCode(String recipientEmail, String verificationCode, LocalDateTime expiresAt) {
        if (recipientEmail == null || recipientEmail.isBlank()) {
            logger.warn("Email verification code dispatch aborted: recipient email is null or empty");
            lastFailure = "Recipient email is null or empty";
            return false;
        }

        if (mailSender == null) {
            logger.warn("JavaMailSender is not configured. Skipping email verification dispatch for recipient: {}", recipientEmail);
            lastFailure = "JavaMailSender not configured";
            return false;
        }

        try {
            logger.info("Initiating account verification email dispatch via Brevo SMTP to recipient: {} [ExpiresAt: {}]", 
                    recipientEmail, expiresAt != null ? expiresAt.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME) : "N/A");
            ...
            helper.setReplyTo(new InternetAddress(fromEmail, fromName, "UTF-8"));
            helper.setTo(recipientEmail);
            helper.setSubject("SkillPilot — Verify Your Account Email");

            String htmlBody = buildEmailVerificationHtml(verificationCode);
            helper.setText(htmlBody, true);
            ...
            this.lastSuccessfulEmail = LocalDateTime.now();
            this.lastFailure = null;
            logger.info("Account verification HTML email successfully delivered via Brevo SMTP to recipient: {} (MessageID: {})", 
                    recipientEmail, messageId != null ? messageId : "generated");
            return true;
        } catch (Exception e) {
            this.lastFailure = e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName();
            logger.error("Failed to send account verification email via Brevo SMTP to recipient {}: {}", recipientEmail, e.getMessage(), e);
            return false;
        }
    }

    private String buildEmailVerificationHtml(String verificationCode) {
        return "<!DOCTYPE html>" +
                "<html lang='en'>" +
                "<head>" +
                "<meta charset='utf-8'>" +
                ...
                "<title>SkillPilot — Verify Your Account</title>" +
                "<style type='text/css'>" +
                ...
                "            <p class='text'>Welcome to SkillPilot! To activate your student workspace, complete your profile, and explore personalized career intelligence matches, enter the 6-digit verification code below:</p>" +
                "            <div class='code-container'>" +
                "              <div class='code-label'>Your Account Verification Code</div>" +
                "              <div class='code'>" + verificationCode + "</div>" +
                "              <div class='expiry-badge'>⏱ Expires in 15 minutes</div>" +
                "            </div>" +
                "            <div class='notice-box'>" +
                "              <p class='notice-title'>Security Notice</p>" +
                "              <p class='notice-text'>If you did not create an account on SkillPilot, you can safely ignore this email. No account will be activated without this verification code.</p>" +
                "            </div>" +
                "          </div>" +
                "          <div class='footer'>" +
